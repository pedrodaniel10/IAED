...
#include "tree.h"
#include "item.h"
/*As funcoes aqui definidas sao baseadas e adaptadas das aulas teoricas*/
...
link procura_no(link noh, Key v){
	if (noh==NULL)
		return NULL;
	else if ( equal(v,key(noh->item)) ) /*Se forem iguais*/
		return noh;
	else if ( less(v, key(noh->item)) ) /*Se v for menor*/
...
link rodar_E(link h){
	...
	prof_esq= profundidade_no(x->esquerda);
